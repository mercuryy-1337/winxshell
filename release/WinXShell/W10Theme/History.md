...
* [wxsUI] 改进UI_Settings组件布局和配色使用Windows11风格，并合并显示系统属性信息。
* [wxsUI] 增加UI_Sample组件，作为UI组件开发实例。
* [wxsUI] 新增自动加载UI组件模式，当执行程序名改为`UI_`开头时, 将自动加载同名UI组件。
  * 可通过在 _WinXShell.lua_ 中指定`App.UIEntryName` 变量的值来改变UI组件的入口配置(默认为`main.jcfg`)。
  * 可通过在 _WinXShell.lua_ 中指定`App.UICmdLine` 变量的值来设置UI组件的额外参数。
